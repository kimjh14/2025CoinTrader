"""
═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
📈 2025CoinTrader - ADA 최적화 백테스팅 시스템 (backtest_ada_optimized.py)
═══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════

🎯 주요 역할:
    • 훈련된 Bi-LSTM 모델을 사용한 ADA 코인 백테스팅
...
🔧 핵심 기능:
    1. 모델 기반 예측: 다중 타임프레임 Bi-LSTM 모델로 ADA 가격 예측
...
📋 사용 방법:
    python backtest_ada_optimized.py
    
    생성 파일:
    - backtest_ada_optimized_YYYYMMDD_HHMMSS.log: 상세 거래 로그
    - ada_backtest_results.png: 백테스팅 결과 차트
    - ada_performance_report.json: 성과 지표 리포트
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'backtest_ada_optimized_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log', encoding='utf-8'),
...
class ADABacktesterOptimized:
    ...
    def __init__(self, initial_capital=10_000_000):
        ...
        logger.info(f"Initializing Optimized ADA Backtester")
    ...
    def load_data(self):
        ...
        # 1분봉 데이터
        ada_1m_path = os.path.join(data_dir, 'ADA_1m.csv')
        self.df_1m = pd.read_csv(ada_1m_path)
        ...
        # 5분봉 데이터
        ada_5m_path = os.path.join(data_dir, 'ADA_5m.csv')
        self.df_5m = pd.read_csv(ada_5m_path)
    ...
    def run_backtest(self):
        """백테스트 실행"""
        logger.info("="*60)
        logger.info("Starting Optimized ADA Backtest")
        logger.info("="*60)
    ...
    def print_results(self):
        ...
        if self.trades:
            ...
            filename = f'backtest_ada_optimized_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
...
if __name__ == "__main__":
    ...
    # 백테스터 생성 및 실행
    backtester = ADABacktesterOptimized(initial_capital=10_000_000)
