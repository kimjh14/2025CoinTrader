...
python tools/backtest.py `
    ...
    --model artifacts/train/model_classic_h20_0.001.joblib `
    --meta artifacts/train/meta_classic_h20_0.001.json `
    --scaler artifacts/train/scaler_classic_h20_0.001.joblib `
    --fee 0.0005 `
    --report artifacts/backtest/bt_classic_h20_0.001.json `
    --log_csv artifacts/backtest/trades_classic_h20_0.001.csv `
    --auto_md `
    --md_out artifacts/backtest/trades_classic_h20_0.001.md
...
def equity_curve(close: np.ndarray, open_prices: np.ndarray, pos: np.ndarray, fee_per_side: float):
    # next-bar execution with open prices
    ret = pd.Series(close).pct_change().fillna(0.0).values
    pos_shift = np.roll(pos, 1)
    pos_shift[0] = 0
    trades = []
    
    # Calculate returns using open prices for entry/exit
    equity = np.ones(len(close))
    for i in range(1, len(close)):
        if pos_shift[i] != 0:
            if pos_shift[i] == pos_shift[i-1]:
                # Position held: use close-to-close return
                daily_ret = (close[i] / close[i-1] - 1) * pos_shift[i]
            else:
                # New position: enter at open
                if i < len(open_prices):
                    daily_ret = (close[i] / open_prices[i] - 1) * pos_shift[i]
                else:
                    daily_ret = ret[i] * pos_shift[i]
                daily_ret -= fee_per_side
                if pos_shift[i-1] != 0:
                    daily_ret -= fee_per_side
        else:
            if pos_shift[i-1] != 0:
                # Exit position at open
                if i < len(open_prices):
                    daily_ret = (open_prices[i] / close[i-1] - 1) * pos_shift[i-1]
                else:
                    daily_ret = 0
                daily_ret -= fee_per_side
            else:
...
def generate_markdown_report(trades, summary_info, output_path):
    ...
    def format_trade_block(i, trade):
        ...
        lines = []
        lines.append(f"**[{i}] {trade['side']} | {entry_time} → {exit_time} (보유 {trade.get('hold_minutes', 0)}분)**")
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        if summary_info['start_time'] and summary_info['end_time']:
            f.write(f"- 기간: {summary_info['start_time']} ~ {summary_info['end_time']}\n")
